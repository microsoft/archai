...
Archai uses a sophisticated YAML based configuration system. As an example, you can [view configuration](https://github.com/microsoft/archai/blob/master/confs/algos/darts.yaml) for running DARTS algorithm. At first it may be bit overwhelming, but this allows to make sure all config parameters are isolated from the code and can freely be changed. The config for search phase is located in `/nas/search` section while for evaluation phase is located in `nas/eval` section. You will observe settings for data loading in `loader` section and training in `trainer` section. You can easily change the number of epochs, batch size etc.

One great thing about Archai config system is that you can override any setting specified in YAML through command line as well. For instance, if you want to run eval only for 200 epochs instead of default 600, specify the path of the value in YAML separated by `.` like this:

```bash
python scripts/main.py --algos darts --nas.eval.trainer.epochs 200
```

You can read more about features available in Archai [config system](conf.md) later.

## Architecture Overview

Now that we are familiar with how to run NAS algorithms, use command line switches and configuration, let's briefly review how Archai works.

Some of the popular NAS methods like DARTS use what is known as *proxy dataset*. So if you want to find network that works on Imagenet, you may use smaller dataset such as CIFAR10 for search. After you have found the network for CIFAR10, you scale it up, change model stems and train it on for longer epochs. This final network now can be used for Imagenet. So these type of NAS methods works in two phases: search and eval.
...
Notice that we create one architecture parameter for each primitive and they stay encapsulated within that instance of `Op` class. The `nn.Module` only has `parameters()` method to retrieve learned weights and does not differentiate between architecture weights vs. the regular weights. The `Op` class however allows to separate these two types of parameters.

Another method to focus is `finalize` which choses top primitives by architecture weight and returns  it.
...
Notice that the parameters of this method tells us the expected input and output shape for each node, the cell type indicating whether its a regular and reduction cell and so on. The core of the method simply creates the `NodeDesc` instances to represent each node.
...
To perform a search, DARTS uses bi-level optimization algorithm. To implement this, we need to separate regular weights from the architecture weights. We then train the architecture weights using the bi-level optimizer. This can be done easily by taking advantage of *hooks* that the trainer provides. These includes `pre_fit` and `post_fit` hooks that gets executed before and after the main code for the `fit` method. So, in the `pre_fit` we can initialize our `BilevelOptimizer` class.
...
```
 ...
 Then we use `pre_step` hook to run the step on `BilevelOptimizer`.
...
Now that we have our own `Trainer` and `ModelDiscBuilder` for DARTS, we need to tell Archai about them. This is done through a class derived from `ExperimentRunner`. We override `model_desc_builder()` and `trainer_class()` to specify our custom classes.
