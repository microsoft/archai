...
class FlopsProfiler(object):
        ...
        def register_module_hooks(module, ignore_list):
            ...
            def post_hook(module, input, output):
                module.__flops__ += sum(
                    [elem[1] for elem in module_flop_count])
                module_flop_count.clear()
    ...
    def print_model_aggregated_profile(self, module_depth=-1, top_modules=3):
        ...
        print(
            f"Top {num_items} modules in flops at depth {depth} are {sort_flops}"
        )
        print(
            f"Top {num_items} modules in params at depth {depth} are {sort_params}"
        )
        print(
            f"Top {num_items} modules in time at depth {depth} are {sort_time}"
        )
...
def _conv_flops_compute(input,
    ...
    output_dims[0] = (input_dims[0] + 2 * paddings[0] -
                      (dilations[0] *
                       (kernel_dims[0] - 1) + 1)) // strides[0] + 1
    output_dims[1] = (input_dims[1] + 2 * paddings[1] -
                      (dilations[1] *
                       (kernel_dims[1] - 1) + 1)) // strides[1] + 1
...
def get_model_profile(
    ...
    for _ in range(warm_up):
        input, target, mem = input_constructor()
        _ = model(input, target, mem)
