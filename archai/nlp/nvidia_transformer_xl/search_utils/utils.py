...
def get_model(model_config, train=False):
    if isinstance(model_config['n_head'], list):# and len(model_config['n_head'])>1:
        ...
        model = MemTransformerLM(**model_config)
        # model_config['n_head'] = [model_config['n_head']] * model_config['n_layer']
        # model_config['d_inner'] = [model_config['d_inner']] * model_config['n_layer']
        # if 'd_head' in model_config.keys():
        #   model_config['d_head'] = [model_config['d_head']] * model_config['n_layer']
        # model = MemTransformerLM_flex(**model_config)
