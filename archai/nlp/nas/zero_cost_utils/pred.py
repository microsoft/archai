...
from archai.nlp.models.mem_transformer.model_mem_transformer import weights_init
...
def get_metrics(topk, sorted_ground_truth, sorted_target, val_ppl_list_gt, val_ppl_list_target, common_configs=None):
    ...
    correct = len(np.intersect1d(
        sorted_target_binned, sorted_ground_truth_binned))
    ...
    topk_val_ppl_list_gt = [val_ppl_list_gt[i] for i in range(
        len(val_ppl_list_gt)) if i in sorted_ground_truth_binned]
    topk_val_ppl_list_target = [val_ppl_list_target[i] for i in range(
        len(val_ppl_list_target)) if i in sorted_ground_truth_binned]
    ...
    print('Spearman Correlation on top %d %% (%d): %.3f' %
...
def get_scores(args, exp_name, tr_iter, method='snip', compute_cost=False):
    ...
    yaml_file = os.path.join(
        path_to_results, f'{method}_scores_seed_{args.seed}.yaml')
    ...
    for _f in set(files):
        if 'model_config.yaml' in _f:
            ...
            measures = predictive.find_measures(args, model, tr_iter, (
                args.dataload, args.dataload_info, args.n_token), args.device, measure_names=[method])
            scores[config_name] = measures[method]
            if compute_cost:
                cost = cost_fn(method, model, tr_iter)
                costs[config_name] = cost
                print(count, config_name,
                        scores[config_name], costs[config_name])
            else:
...
def get_statistics(method, results_gt, scores, nparams_dict, topk_list):
    ...
    ax1.plot(range(len(val_ppl_list_gt)), np.asarray(
        val_ppl_list_gt)[sorted_ground_truth], label='val_ppl')
    ax2 = ax1.twinx()
    ax2.plot(range(len(val_ppl_list_gt)), np.asarray(target_scores)
             [sorted_ground_truth], label='# decoder params')
    plt.legend()
...
def plot(args, methods):
    ...
    for lk in legend_keys:
        for i, m in enumerate(common_ratios[lk].keys()):
            ...
            plt.scatter(range(1, len(param_types)+1),
                        corrs, label=m)  # lk+'_method_'+m)
    ...
    plt.grid(axis='y')
    plt.savefig(
        f'common_ratio_topk_zero-cost_{exp_name}.png', bbox_inches="tight")
    ...
    for lk in legend_keys:
        for i, m in enumerate(common_ratios[lk].keys()):
            ...
            # lk+'_method_'+m)
            plt.scatter(
                avg_cost, spr_ranks[lk][m][-1], label=l, marker=markers[i], s=180, c=colors[i])
    plt.ylabel('Spearman\'s Correlation')
...
def cost_fn(method, model, tr_iter):
    ...
    with torch.no_grad():
        for _, (inp, tgt, _, _) in enumerate(tr_iter):
            ...
            total_flops = np.sum([curr_flops[k]
                                 for k in curr_flops.keys()]).tolist()
            break
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Zero-cost Metrics for Transformers')
    parser.add_argument('--exp_name', type=str, default='/home/gderosa/archai/amlt/',
                        help='path to the experiment')
    parser.add_argument('--get_cost', action='store_true',
                        help='compute cost for each method')
    parser.add_argument('--seed', type=int, default=1111, help='Random seed')
    parser.add_argument('--plot', action='store_true',
                        help='plot the spearman corr and common ratio')
    parser.add_argument('--method', type=str, default='snip',
                        help='zero-cost method to use')
    parser.add_argument('--cuda', action='store_true',
                        help='use gpu for score calculation')

    parser.add_argument('--batch_size', type=int,
                        default=16, help='Global batch size')
    parser.add_argument('--dataset', type=str, default='wt103',
                        choices=['wt103', 'lm1b'], help='Dataset name')
    parser.add_argument('--vocab', type=str, default='gpt2',
                        choices=['gpt2'], help='Type of vocabulary')
    parser.add_argument('--vocab_size', type=int,
                        default=None, help='Size of vocabulary')
    parser.add_argument('--dataload', type=str,
                        default='random', help='random or grasp supported')
    ...
    parser.add_argument('--write_freq', type=int, default=100,
                        help='frequency of write to file')
...
if __name__ == '__main__':
    ...
    data, work_dir, _, cache_dir, dataroot = exp_utils.get_create_dirs(None,
                                                                       args.dataset,
                                                                       pretrained_path='',
                                                                       cache_dir='cache')

    corpus = get_lm_corpus(data,
                           cache_dir,
                           args.dataset,
                           'gpt2',
                           vocab_size=50257,
                           refresh_cache=False)
    train_itr = corpus.get_iterator('train',
                                    eval_batch_size,
                                    eval_tgt_len,
                                    device=device,
                                    mem_len=0,
    ...
    for method in methods:
        get_scores(args, args.exp_name, train_itr, method=method,
                       compute_cost=args.get_cost)
