...
def find_measures_arrays(args, net_orig, trainloader, dataload_info, device, measure_names=None, loss_fn=F.cross_entropy):
    ...
    if dataload == 'random':
        inputs, targets = get_some_data(args, trainloader, num_batches=num_imgs_or_batches, device=device)
    elif dataload == 'grasp':
        inputs, targets = get_some_data(args, trainloader, num_batches=num_imgs_or_batches, device=device)
...
def find_measures(args, net_orig,                  # neural network
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(args, net_orig, dataloader, dataload_info, device, loss_fn=loss_fn, measure_names=measure_names)
