...
yaml.add_constructor(
    u'tag:yaml.org,2002:python/object/apply:numpy.core.multiarray.scalar', meta_constructor_sequence)
yaml.add_constructor(
    u'tag:yaml.org,2002:python/object/apply:numpy.dtype', meta_constructor_mapping)
...
def _forward_synflow_memformer(self, dec_inp, mems=None):
    ...
    if self.same_length:
        ...
        dec_attn_mask = torch.triu(word_emb.new_ones(
            qlen, klen), diagonal=1+mlen).bool()
    ...
    if self.attn_type == 0:
        ...
        core_out = torch.ones_like(
            core_out, dtype=core_out.dtype, device=core_out.device)
        pos_emb = torch.ones_like(
            pos_emb, dtype=pos_emb.dtype, device=pos_emb.device)
...
def _forward_synflow_memformer_flex(self, dec_inp, mems=None):
    ...
    if self.attn_type == 0:
        ...
        for i, layer in enumerate(self.layers):
            ...
            core_out = layer(core_out, pos_emb, getattr(self, f'r_w_bias_{i}'), getattr(
                self, f'r_r_bias_{i}'), dec_attn_mask=dec_attn_mask, mems=mems_i)
    else:
        raise NotImplemented
...
def forward_synflow(self, data, target, mems, output_loss=True, output_prediction_scores=False):    
    # Causal attention mask is created inside the model
    outputs = self.model(input_ids=data,
                         labels=target,
...
def forward_crit(self, hidden, target=None, keep_order=False):
    ...
    if self.n_clusters == 0:
        ...
        head_weight, head_bias, head_proj = weights[0], biases[0], self.get_out_proj(
            0)
            ...
            #                         'Attn': float(params_attention), 'FFN': float(params_ff), 'total': float(curr_n_all_param)}
                ...
                if calc_scores:
                    for idx, (inp, tgt, seqlen, _) in enumerate(diter):
                        ...
                        score = np.sum([torch.sum(g).detach().numpy()
                                    for g in grads_abs])
                        break
                ...
                if calc_costs:
                    ...
                    with torch.no_grad():
                        for _, (inp, tgt, _, _) in enumerate(diter):
                            ...
                            total_flops = np.sum(
                                [curr_flops[k] for k in ['Attn', 'FFN', 'Sftmax']]).tolist()
                            break
...
def get_statistics(seed, results_gt, scores, nparams_dict, topk_list):
    common_configs = np.intersect1d(
        list(results_gt.keys()), list(scores[seed].keys()))
...
def plot(args):
    ...
    for lk in legend_keys:
        for seed in common_ratios[lk].keys():
            plt.scatter(
                topk_list, common_ratios[lk][seed], label=lk+'_seed_'+seed)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cross_seed', action='store_true',
