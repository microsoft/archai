...
def get_parser():
  ...
  parser.add_argument('--lr_sweep', action='store_true',
                      help='measure correlation between parameter count and validation ppl before and after lr sweep') 
  parser.add_argument('--bert_finetuning', action='store_true',
                      help='measure correlation between parameter count and vbert finetuning results on glue datasets') 
...
def main(args):
  if args.read_jsons: # extracts information from the .json training log (see get_info_from_json function above)
    ...
    for exp_name in args.exp_name:
      ...
      if os.path.exists(yaml_file):
        ...
        for config_name, model_config in model_configs.items():
          ...
          #   if 'tie_projs' not in model_config:
          #     model_config['cutoffs'] = [19997, 39997, 199997] if model_config['n_token'] == 267736 else [59997, 99997, 639997]
          #     model_config['tie_projs'] = [False] + [True] * len(model_config['cutoffs'])
          #     model_config['tie_weight'] = True
    ...
    for exp_name in args.exp_name:
      ...
      path_to_results = os.path.join(args.results_dir, exp_name)
      yaml_file = os.path.join(path_to_results, 'result_summary.yaml')#'ppl_summary.yaml')
      with open(yaml_file, 'r') as f:
      ...
      common_configs = np.intersect1d(list(results_gt.keys()), list(n_all_params.keys()))[:200]
      print('analyzing {} architectures'.format(len(common_configs)))
      ...
      for k in common_configs:
        ...
        #   continue
        if np.min(model_configs[k]['d_inner']) < 1.7 * model_configs[k]['d_model']:
          continue
        ...
        n_layers[legend_key].append(model_configs[k]['n_layer'])
      ...
      print('decoder param count range:', np.min(np.absolute(n_params[legend_key])[sorted_nparams]), 
                                        np.max(np.absolute(n_params[legend_key])[sorted_nparams]))
      print('total param count range:', np.min(np.absolute(n_params_total[legend_key])[sorted_nparams_total]), 
                                        np.max(np.absolute(n_params_total[legend_key])[sorted_nparams_total]))
      print('div_vals:', np.unique(div_vals[legend_key]))
      print('layer count range:', np.min(n_layers[legend_key]), np.max(n_layers[legend_key]))
    ...
    for k in legend_keys:
      plt.plot(topk_list, common_ratios[k], marker='.', markersize=12, label='CR - DecoderParams', color='tab:blue')
      plt.plot(topk_list, spr_ranks[k], marker='^', markersize=10, label='SRC - DecoderParams', color='midnightblue')
      plt.plot(topk_list, common_ratios_total[k], marker='.', linestyle='--',markersize=12, label='CR - TotalParams', color='tab:blue')
      plt.plot(topk_list, spr_ranks_total[k], marker='^', linestyle='--', markersize=10, label='SRC - TotalParams', color='midnightblue')
    # plt.ylabel('Common ratio')
    plt.xlabel('Topk (%)')
    plt.xticks(topk_list)
    # plt.title('ranking based on number of parameters')
    ...
    for exp_name in args.exp_name:
      ...
      path_to_results = os.path.join(args.results_dir, exp_name)
      yaml_file = os.path.join(path_to_results, 'ppl_summary.yaml')#'result_summary.yaml')
      with open(yaml_file, 'r') as f:
    ...
    for k in common_configs:
      ...
      n_layers.append(model_configs[k]['n_layer'])
      d_models.append(model_configs[k]['d_model'])

    # sorted_ground_truth = np.argsort(val_ppl_list_gt)[::-1][:200]
    # val_ppl_list_gt = [val_ppl_list_gt[i] for i in range(len(val_ppl_list_gt)) if i in sorted_ground_truth]
    # n_params = [n_params[i] for i in range(len(n_params)) if i in sorted_ground_truth]
    # n_params_total = [n_params_total[i] for i in range(len(n_params_total)) if i in sorted_ground_truth]

    sorted_ground_truth = np.argsort(val_ppl_list_gt)
    sorted_nparams = np.argsort(n_params)
    sorted_nparams_total = np.argsort(n_params_total)
    print('analyzing {} architectures'.format(len(sorted_ground_truth)))
    ...
    print('val_ppl range:', np.min(val_ppl_list_gt), np.max(val_ppl_list_gt))
    print('decoder param count range:', np.min(np.absolute(n_params)), 
                                        np.max(np.absolute(n_params)))
    print('total param count range:', np.min(np.absolute(n_params_total)), 
                                      np.max(np.absolute(n_params_total)))
    print('div_vals:', np.unique(div_vals))
    print('layer count range:', np.min(n_layers), np.max(n_layers))
    ...
    colors = ['tab:blue', 'tab:green', 'tab:orange']#plt.cm.Blues(np.linspace(0.5, 1, 3))
    ...
    for subcolor in np.unique(c):
      plt.scatter(-np.asarray(n_params_total)[sorted_ground_truth][np.asarray(c)==subcolor], np.asarray(val_ppl_list_gt)[sorted_ground_truth][np.asarray(c)==subcolor], s=25, c=subcolor, label=r'$k$={}'.format(unique_div_vals[colors.index(subcolor)]))
    plt.ylabel('Validation PPL')
    plt.xlabel('Total nParams')
    plt.grid(axis='y')
    plt.ylim(bottom=24, top=51)
    # plt.legend()
    plt.savefig('pareto_params_total.png', bbox_inches="tight")

    plt.figure(figsize=(7,4.2))
    plt.plot(topk_list, common_ratios, marker='.', markersize=12, label='CR - DecoderParams')
    plt.plot(topk_list, spr_ranks, marker='^', markersize=10, label='SRC - DecoderParams', color='midnightblue')
    plt.plot(topk_list, common_ratios_total, marker='.', markersize=12, label='CR - TotalParams', color='tab:blue', linestyle='--')
    plt.plot(topk_list, spr_ranks_total, marker='^', markersize=10, label='SRC - TotalParams', color='midnightblue', linestyle='--')
    plt.xlabel('Topk (%)')
    plt.xticks(topk_list)
    plt.grid(axis='y')
    # plt.ylim((0.4,1.01))
    ...
    # find points with similar archs and ppls but different param counts
    # for i, n_param1 in enumerate(n_params):
    #   for j, n_param2 in enumerate(n_params[i+1:]):
    #     if abs(val_ppl_list_gt[i]-val_ppl_list_gt[j])/val_ppl_list_gt[i]<0.01 and n_layers[i]==n_layers[j] and d_models[i]==d_models[j] and div_vals[i]!=div_vals[j]:
    #     # if abs(n_param1-n_param2)/abs(n_param1)<0.01 and n_layers[i]==n_layers[j] and div_vals[i]!=div_vals[j]:
    #       print('div_val:', div_vals[i], div_vals[j])
    #       print('params:', abs(n_param1), abs(n_param2))
    #       print('total params:', abs(n_params_total[i]), abs(n_params_total[j]))
    #       print('val_ppl:', val_ppl_list_gt[i], val_ppl_list_gt[j])
    #       print('------------------')
    ...
    path_to_results = os.path.join(args.results_dir, args.exp_name[0])
    all_dirs = os.listdir(path_to_results)
    
    orig_results = {}
    sweep_results = {}
    for dir in all_dirs:
      config_name = dir.split('_')[-1]
      if 'lr_sweep' in dir:
        for subdir in os.listdir(os.path.join(path_to_results, dir)):
          curr_lr = subdir.split('_')[-1]
          for job_name in os.listdir(os.path.join(path_to_results, dir, subdir)):
            key = f'config{config_name}_{job_name}'
            
            logs = read_yaml(os.path.join(path_to_results, dir, subdir, job_name, 'summary.yaml'))
            if logs is None:
              print("no logs found for", os.path.join(dir, subdir, job_name))
            else:
              if key in sweep_results:
                sweep_results[key][curr_lr] = logs
              else:
                sweep_results[key] = {curr_lr: logs}

      else:
        for job_name in os.listdir(os.path.join(path_to_results, dir, dir)):
          key = f'config{config_name}_{job_name}'
          logs = read_yaml(os.path.join(path_to_results, dir, dir, job_name, 'summary.yaml'))
          if logs is not None:
            assert not key in orig_results
            orig_results[key] = logs
          else: 
            print("no logs found for", os.path.join(dir, dir, job_name))
          
    common_configs = np.intersect1d(list(orig_results.keys()), list(sweep_results.keys()))
    print(f'summarizing {len(common_configs)} configs')

    n_params_baseline = np.asarray([orig_results[k]['n_nonemb_param'] for k in common_configs])
    val_ppl_baseline = [orig_results[k]['valid_ppl'] for k in common_configs]
    ...
    for k in common_configs:
      print(k)
      n_layers.append(orig_results[k]['n_layer'])
      n_total_params.append(orig_results[k]['n_all_param'])
      n_params_orig.append(orig_results[k]['n_nonemb_param'])
      val_ppl_orig.append(orig_results[k]['valid_ppl'])
      sweep_results[k]['baseline_lr'] = orig_results[k]

      lrs = list(sweep_results[k].keys())

      if k=='config15_j8':
        lrs.remove('lr0.1000')


      n_params_sweep.append(sweep_results[k][lrs[0]]['n_nonemb_param'])
      for lr in lrs:
        assert sweep_results[k][lr]['n_nonemb_param'] == n_params_orig[-1]

      min_idx = np.argmin([sweep_results[k][lr]['valid_ppl'] for lr in lrs])
      val_ppl_sweep.append(np.min([sweep_results[k][lr]['valid_ppl'] for lr in lrs]))
      if lrs[min_idx] != 'baseline_lr':
        print(f'#################### best lr={lrs[min_idx]}')
        print(sweep_results[k][lrs[min_idx]]['valid_ppl'], sweep_results[k]['baseline_lr']['valid_ppl'])

    rho, p = spearmanr(n_params_orig, val_ppl_orig)
    print(f"Original correlation for decoder params = {rho}, {p}")
    rho, p = spearmanr(n_params_sweep, val_ppl_sweep)
    print(f"Sweeped correlation for decoder params = {rho}, {p}")

    sorted_baseline = np.argsort(val_ppl_orig)
    sorted_gt = np.argsort(val_ppl_sweep)
    rank_gts = []
    for idx in sorted_baseline:
      rank_gt = np.where(sorted_gt==idx)[0][0]
      rank_gts.append(rank_gt)

    print('nonemb param range:', np.min(n_params_orig), np.max(n_params_orig))
    print('total param range:', np.min(n_total_params), np.max(n_total_params))
    print('n_layer range:', np.min(n_layers), np.max(n_layers))

    plt.figure(figsize=(3.5,3.5))
    plt.scatter(range(1, len(sorted_baseline)+1), np.asarray(rank_gts)+1, s=8)
    plt.plot(range(1, len(sorted_baseline)+1), range(1, len(sorted_baseline)+1), linestyle='dashed', color='black')
    plt.xlabel('Const. LR Ranking')
    plt.ylabel('Optim. LR Ranking')
    # plt.xlim((-1, 200))
    # plt.ylim((-1, 200))
    plt.savefig('params_corr_lr_sweep.png', bbox_inches="tight")

  elif args.bert_finetuning:
    dataset_info = {'cola': {'log_fname': 'trainer_state.json', 'metric': 'eval_obj_0.matthews_correlation'},
                'mnli': {'log_fname': 'all_results.json', 'metric': 'val_matched_eval_obj_0.accuracy'},#, 'val_mismatched_eval_obj_0.accuracy']},
                'mrpc': {'log_fname': 'all_results.json', 'metric': 'test_eval_obj_0.f1'},
                'qnli': {'log_fname': 'trainer_state.json', 'metric': 'eval_obj_0.accuracy'},
                'qqp': {'log_fname': 'trainer_state.json', 'metric': 'eval_obj_0.f1'}}

    all_results_dict = {}
    
    results_path = os.path.join(args.results_dir, args.exp_name[0], '**', 'params_results.json')
    param_files = glob.glob(results_path, recursive=True)
    
    for param_file in param_files:
      with open(param_file, 'r') as f:
        params = json.load(f)
      path = os.path.abspath(os.path.join(param_file, os.pardir))
      dset_name = path.split('/')[-1].split('-')[1]
      log_file = glob.glob(os.path.join(path, '**', dataset_info[dset_name]['log_fname']), recursive=True)
      assert len(log_file)

      with open(log_file[0], "r") as f:
        output_dict = json.load(f)
      try:
        log_history  = output_dict['log_history'][-1]
        assert log_history['epoch'] == 3, path   #make this adaptive based on the yaml file
      except:
        log_history = output_dict
      metric = log_history[dataset_info[dset_name]['metric'].split('.')[0]][dataset_info[dset_name]['metric'].split('.')[1]]
      
      if dset_name in all_results_dict:
        all_results_dict[dset_name]['n_parameters'].append(params['n_parameters'])
        all_results_dict[dset_name]['n_non_embedding_parameters'].append(params['n_non_embedding_parameters'])
        all_results_dict[dset_name]['metric'].append(metric)
      else:
        all_results_dict[dset_name] = {}
        all_results_dict[dset_name]['n_parameters'] = [params['n_parameters']]
        all_results_dict[dset_name]['n_non_embedding_parameters'] = [params['n_non_embedding_parameters']]
        all_results_dict[dset_name]['metric'] = [metric]

    for dset_name in dataset_info.keys():
      print('-------------------------- found {} log files for {}'.format(len(all_results_dict[dset_name]['n_parameters']), dset_name))

      print('n_parameters:')
      common_ratio_total, spr_rank_total = get_metrics(topk=100, sorted_ground_truth=np.argsort(all_results_dict[dset_name]['metric']), 
                                                      sorted_target=np.argsort(all_results_dict[dset_name]['n_parameters']), \
                                                      val_ppl_list_gt=all_results_dict[dset_name]['metric'], 
                                                      val_ppl_list_target=all_results_dict[dset_name]['n_parameters'])
      print('n_non_embedding_parameters:')
      common_ratio_total, spr_rank_total = get_metrics(topk=100, sorted_ground_truth=np.argsort(all_results_dict[dset_name]['metric']), 
                                                      sorted_target=np.argsort(all_results_dict[dset_name]['n_non_embedding_parameters']), \
                                                      val_ppl_list_gt=all_results_dict[dset_name]['metric'], 
                                                      val_ppl_list_target=all_results_dict[dset_name]['n_non_embedding_parameters'])

      plt.figure(figsize=(3.5,3.5))
      plt.scatter(np.sort(all_results_dict[dset_name]['metric']), np.asarray(all_results_dict[dset_name]['n_parameters'])[np.argsort(all_results_dict[dset_name]['metric'])], s=8)
      plt.xlabel('n_parameters')
      plt.ylabel(dataset_info[dset_name]['metric'].split('.')[-1])
      plt.savefig(f'total_params_vs_metric_{dset_name}.png', bbox_inches="tight")

      plt.figure(figsize=(3.5,3.5))
      plt.scatter(np.sort(all_results_dict[dset_name]['metric']), np.asarray(all_results_dict[dset_name]['n_non_embedding_parameters'])[np.argsort(all_results_dict[dset_name]['metric'])], s=8)
      plt.xlabel('n_non_embedding_parameters')
      plt.ylabel(dataset_info[dset_name]['metric'].split('.')[-1])
      plt.savefig(f'nonemb_params_vs_metric_{dset_name}.png', bbox_inches="tight")
