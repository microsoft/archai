...
"""Wrapper for Text Predict compliant models (PyTorch and ONNX).
"""
...
class TextPredictModel:
    ...
    def __init__(self, model_type: str, model_path: str, space_token_id: int, max_seq_len: int):
        ...
        with torch.no_grad():
            ...
            for idx in range(0, len(input_ids) - 1, self.max_seq_len):
                ...
                loss, _, _, *_ = self.model(
                    input_ids_t, labels=labels_t, mems=None, output_loss=True, output_prediction_scores=False
                )
    ...
    def get_probs(self, input_ids: Tuple[int, ...]) -> List[float]:
        ...
        with torch.no_grad():
            _, prediction_scores, _, *_ = self.model(
                input_ids_t, labels=None, mems=None, output_loss=False, output_prediction_scores=True
            )
...
class TextPredictTorchModel(TextPredictModel):
    ...
    def __init__(
        self, model_type: str, model_path: str, space_token_id: int, max_seq_len: int, device: Optional[str] = None
    ) -> None:
...
class TextPredictONNXModel(TextPredictModel):
    ...
    def __init__(self, model_type: str, model_path: str, space_token_id: int, max_seq_len: int) -> None:
        super().__init__(model_type, model_path, space_token_id, max_seq_len)
        ...
        # Creates the session
        self.session = ort.InferenceSession(model_path, self.sess_options, providers=["CPUExecutionProvider"])
