...
"""Wrapper for Text Predict tokenization-based utilities.
"""
...
class TextPredictTokenizer:
    ...
    def __init__(self, vocab_path: str) -> None:
        ...
        self.TOKENIZER_WORD_TOKEN_SEPARATOR = set(
            [idx for idx in range(len(self)) if self[idx][0] in TOKENIZER_WORD_TOKEN_SEPARATOR_SET]
        )
    ...
    def upper_tokens(self) -> Set:
        if not hasattr(self, "_upper_tokens"):
            self._upper_tokens = {
                ...
                if any([c.isupper() and c not in TOKENIZER_WORD_TOKEN_SEPARATOR_SET for c in self[idx]])
            }  # pylint: disable=attribute-defined-outside-init

        return self._upper_tokens

    def _filter_token_ids(self, filter_prefix: str, filter_token_ids_cache: LRUCache) -> Tuple[int, ...]:
        ...
        cached_filter_prefix = next(
            (
                ...
                if filter_prefix[:i] in filter_token_ids_cache
            ),
            None,
        )
        ...
        if len(filter_prefix) > 0 and cached_filter_prefix is not None:
            ...
            idx_token_result = [
                (idx, token)
                for idx, token in prefilter_full_result
