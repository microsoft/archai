...
"""Wrapper for Predictor functionalities.
"""
...
class Prediction:
    ...
    @classmethod
    def next_prediction(
        cls: Prediction, prediction: Prediction, next_text: str, next_probability: float, next_token_id: int
    ) -> Prediction:
    ...
    def score(self) -> float:
        ...
        if self.predictor is None:
            if not self.is_empty():
                logging.info(
                    "Prediction environment not available. Ignoring additional parameters needed for score evaluation."
                )
        ...
        if len(upper_token_ids) > 0:
            score = (self.probability - Prediction.UPPER_PROB_PENALTY) * length * (
                a1 * length + b1
            ) - Prediction.UPPER_SCORE_PENALTY
...
class TextPredictionPosition:
    ...
    @classmethod
    def from_smart_compose_ljson(cls: TextPredictionPosition, line: str) -> TextPredictionPosition:
        d = json.loads(line)
    ...
    def to_smart_compose_ljson(self) -> str:
        result = OrderedDict({"UniqueId": self.unique_id, "Body": self.body, "BodyContinued": self.body_continued})
...
class TextPredictionSequence(OrderedDict):
    ...
    @classmethod
    def from_file(
        cls: TextPredictionSequence, file_name: str, file_type: str, predictor: Optional[Predictor] = None, **kwargs
    ...
    @classmethod
    def from_smart_compose_file(
        cls: TextPredictionSequence, file_name: str, predictor: Optional[Predictor] = None, **kwargs
    ) -> TextPredictionSequence:
    ...
    def save_all(
        ...
        triggered_file: Optional[str] = "triggered.csv",
    ) -> None:
    ...
    def get_predictions(self) -> pd.Series:
        ...
        for pos in self.values():
            ...
            if prediction is not None:
                ...
                last_match_char = next(
                    (i for i in range(min_len) if prediction.show()[i] != body_continued[i]), min_len
                )
        ...
        predictions_df_columns = (
            predictions_df_columns
            + predictions_df.columns.drop(predictions_df_columns + ["Complete", "Tokens"]).tolist()
        )
    ...
    def score_triggered_predictions(self, triggered_df: pd.DataFrame, min_score: Optional[float] = None) -> OrderedDict:
        summary = OrderedDict()
    ...
    def score(self, min_scores: Union[list, float], expected_match_rate: Optional[float] = None) -> List[Any]:
        ...
        for min_score in min_scores:
            if min_score is None:
                ...
                if len(match_rates) < 2:
                    logging.warning(
                        f"Not enough points to calculate score for the expected match rate: {expected_match_rate}."
                    )
                    continue
...
class Predictor:
    ...
    def filter_next_tokens(
        self, input_ids: Optional[Tuple[int, ...]] = (), filter_prefix: Optional[str] = ""
    ) -> Tuple[int, ...]:
        ...
        if filter_prefix_len == 0:
            ...
            result = [
                (tuple_idx, next_token_probs[tuple_idx[0]], len(self.tp_tokenizer[tuple_idx[0]]) - filter_prefix_len)
    ...
    @functools.lru_cache(maxsize=1024)
    def filter_next_tokens_extended(
        self,
        input_ids: Tuple[int, ...],
        ...
        if idxs is None:
            if global_prob != 1.0:
                filtered_next_token = [
                    (idx, prob * global_prob, extra_token_len) for idx, prob, extra_token_len in filtered_next_token
                ]
            else:
                filtered_next_token = copy.copy(filtered_next_token)
        else:
            filtered_next_token = [
                (idxs + idx, prob * global_prob, extra_token_len) for idx, prob, extra_token_len in filtered_next_token
            ]

        return filtered_next_token

    def predict_word_prefix(self, input_ids: Tuple[int, ...], prefix: str, debug: Optional[bool] = False) -> Prediction:
        if len(prefix) > self.PREFIX_MAX_LEN:
            logging.debug(
                "predict_word_prefix: prefix: `%s` of length %s is longer than %s chars",
                prefix,
                len(prefix),
                self.PREFIX_MAX_LEN,
            )
        ...
        logging.debug(
            "predict_word_prefix: prefix: `%s`: filtered_list[:5] %s elem: %s; %.3f ms",
            prefix,
            len(filtered_list),
            filtered_list[:5],
        ...
        while calc_count < Predictor.PREFIX_MAX_NEXT_CANDIDATE_CALC:
            ...
            idx_with_reminder = next(
                (
                    i
                    for i, elems in enumerate(filtered_list)
                    if elems[2] < 0
                    and elems[1] > Predictor.PREFIX_MIN_NEXT_CANDIDATE_PROB
            ...
            if (
                idx_with_reminder > 0
                and filtered_list[0][1] * Predictor.PREFIX_MIN_NEXT_CANDIDATE_PROB_FRAC
                > filtered_list[idx_with_reminder][1]
            ):
            ...
            filtered_token_prob_with_reminder = self.filter_next_tokens_extended(
                tuple(input_ids + idxs), token_reminder, tuple(idxs), prob
            )
            ...
            if logging.getLogger().level == logging.DEBUG:
                ...
                logging.debug(
                    "#%d: %.3f ms: %.3g %s", calc_count, 100 * (time.time() - while_start), prob_sum, filtered_list_cond
                )
        ...
        if len(filtered_list) > 0 and filtered_list[0][2] >= 0:
            ...
            logging.debug(
                "predict_word_prefix: prefix: `%s`: # calcs: %s; time: %.3f ms",
                prefix,
                calc_count,
                1000 * (time.time() - start_time),
            )
    ...
    def is_complete_word(self, input_ids: Tuple[int, ...]) -> bool:
        ...
        prob_sum = sum(
            [prob for idx, prob in enumerate(probs) if idx in self.tp_tokenizer.TOKENIZER_WORD_TOKEN_SEPARATOR]
        )
    ...
    def predict_full(self, text: str) -> Tuple[Prediction, list]:
        ...
        logging.debug(
            "Predictor.predict_full: context[-20:]: `%s`; " + "input_ids[-5:]: %s; prefix: `%s`; time: %.3f ms",
            context[-20:],
            input_ids[-5:],
            prefix,
            1000 * (time.time() - start_time),
        ...
        while total_prob > self.MIN_PROB_CUTOFF and calc_count < self.MAX_TOKEN_CALC:
            ...
            if (
                len(prediction) >= self.MIN_PRED_LEN
...
def run_score(
    ...
    seq = (TextPredictionSequence).from_file(
        input_file_path,
        input_file_type,
        predictor,
        save_step=save_step,
