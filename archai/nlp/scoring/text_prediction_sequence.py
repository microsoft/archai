...
from archai.nlp.scoring.text_predictor import Prediction, TextPredictor
...
print(json.dumps(score, indent=2))
    ...
    @classmethod
    def from_file(cls, file_name: str, file_type: str, predictor:Optional[TextPredictor] = None)->TextPredictionSequence:
        ...
        raise NotImplementedError(f'file_type {file_type} is not implemented')
    ...
    def from_text_file(cls, file_name: str, new_document_re: str = "\\n\\n+", predictor:Optional[TextPredictor] = None) -> TextPredictionSequence:
        ...
        for line_id, line in enumerate(tqdm.tqdm(lines)):
            line = line.strip()
    ...
    def save(self, output_filepath: str) -> None:
        ...
        """
        output = [pos.to_smart_compose_ljson() + "\n" for pos in tqdm.tqdm(self.values(), desc="Save", disable=self.SILENT)]
    ...
    def predict(self, output_filepath: str = None) -> None:
        ...
        for i, pos in enumerate(tqdm.tqdm(self.values(), desc="Predict", disable=self.SILENT)):
    ...
    def get_perplexity(self):
        ...
        loss_sum = 0.0
        token_ids_len_sum = 0
        for unique_id in tqdm.tqdm(self.filter_keys_char_id(1), desc="Perplexity", disable=self.SILENT):
            text = self[unique_id].body + self[unique_id].body_continued
            text = self.predictor.tokenizer.clean(text)
            token_ids = self.predictor.tokenizer.encode(text)
    ...
    def get_predictions(self) -> pd.DataFrame:
        ...
        """
        predictions_list = []
        for pos in tqdm.tqdm(self.values(), desc="Parsing", disable=self.SILENT):
    ...
    def calc_triggered_predictions(self, min_score: float, predictions_df: pd.DataFrame = None) -> pd.DataFrame:
        ...
        for i in tqdm.tqdm(range(predictions_df.shape[0]), desc=f"Score: {min_score:.3g}", disable=self.SILENT):
