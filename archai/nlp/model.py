...
"""Customizable automatic models like huggingface/transformers.
"""
...
MODELS = {
    "bert": {
        "clm": ("BERTConfig", "BERTForCLM"),
        "mlm": ("BERTConfig", "BERTForMLM"),
        "pre_train": ("BERTConfig", "BERTForPreTraining"),
        "seq_clf": ("BERTConfig", "BERTForSequenceClassification"),
    },
    "codegen": {"clm": ("CodeGenConfig", "CodeGenForCLM")},
    "ctrl": {"clm": ("CTRLConfig", "CTRLForCLM")},
    "gpt": {"clm": ("GPTConfig", "GPTForCLM")},
    "gpt2": {"clm": ("GPT2Config", "GPT2ForCLM")},
    "gpt2_flex": {"clm": ("GPT2FlexConfig", "GPT2FlexForCLM")},
    "longformer": {"mlm": ("LongformerConfig", "LongformerForMLM")},
    "opt": {"clm": ("OPTConfig", "OPTForCLM")},
    "opt_flex": {"clm": ("OPTFlexConfig", "OPTFlexForCLM")},
    "reformer": {"clm": ("ReformerConfig", "ReformerForCLM")},
    "roberta": {
        "clm": ("RobertaConfig", "RobertaForCLM"),
        "mlm": ("RobertaConfig", "RobertaForMLM"),
        "seq_clf": ("RobertaConfig", "RobertaForSequenceClassification"),
    },
    "roformer": {"clm": ("RoFormerConfig", "RoFormerForCLM")},
    "transfo_xl": {"clm": ("TransfoXLConfig", "TransfoXLForCLM")},
    "xlm": {"clm": ("XLMConfig", "XLMForCLM")},
    "xlnet": {"clm": ("XLNetConfig", "XLNetForCLM")},
}


def get_layers_from_module(module: torch.nn.Module, layer_type: Optional[str] = None) -> List[torch.nn.Module]:
...
class ArchaiModel:
    ...
    def from_config(cls: ArchaiModel, **kwargs) -> ArchaiModel:
        ...
        if model_task in models.keys():
            ...
            raise NotImplementedError(f"model_task: {model_task} has not been implemented yet.")
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model_task = config.model_task.replace("-", "_") if getattr(config, "model_task", None) else "clm"
        if model_task in models.keys():
            model = models[model_task]
        else:
            raise NotImplementedError(f"model_task: {model_task} has not been implemented yet.")
        ...
        model = model_cls.from_pretrained(pre_trained_model_path, *args, config=config, **kwargs)
