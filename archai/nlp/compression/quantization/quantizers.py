...
class FakeDynamicQuantPython(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.register_full_backward_hook(FakeDynamicQuantPython.backward_hook)

    @staticmethod
    def backward_hook(module, grad_input, grad_output):
        return (module.mask*grad_output[0],)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dtype == torch.float32:
            ...
            scalei = float(scale.item())
            zero_pointi = int(zero_point.item())

            inv_scale = 1.0 / scalei
