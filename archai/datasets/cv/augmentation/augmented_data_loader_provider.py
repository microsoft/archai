...
from archai.supergraph.datasets.limit_dataset import DatasetLike, LimitDataset
...
def get_dataloaders(
    ...
    logger.info({"train_workers": train_workers, "val_workers": val_workers, "test_workers": test_workers})

    transform_train, transform_test = ds_provider.get_transforms(img_size)
    add_named_augs(transform_train, aug, cutout)

    trainset, testset = _get_datasets(ds_provider, load_train, load_test, transform_train, transform_test)
    ...
    if trainset:
        ...
        logger.info({"val_ratio": val_ratio, "max_train_batches": max_batches, "max_train_fold": max_train_fold})

        # sample validation set from trainset if cv_ratio > 0
        train_sampler, valid_sampler = _get_sampler(
            trainset, val_ratio=val_ratio, shuffle=True, apex=apex, max_items=max_train_fold
        )
        logger.info(
            {
                "train_sampler_world_size": train_sampler.world_size,
        ...
        if valid_sampler:
            logger.info(
                {
                    "valid_sampler_world_size": valid_sampler.world_size,
        ...
        trainloader = DataLoader(
            ...
            drop_last=False,
        )  # TODO: original paper has this True
    ...
    if testset:
        ...
        logger.info({"max_test_batches": max_batches, "max_test_fold": max_test_fold})

        test_sampler, test_val_sampler = _get_sampler(
            testset, val_ratio=None, shuffle=False, apex=apex, max_items=max_test_fold
        )
        logger.info(
            {
                "test_sampler_world_size": test_sampler.world_size,
...
def _get_datasets(
    ds_provider: DatasetProvider, load_train: bool, load_test: bool, transform_train, transform_test
) -> Tuple[DatasetLike, DatasetLike]:

    trainset, testset = ds_provider.get_datasets(load_train, load_test, transform_train, transform_test)
    return trainset, testset


# target_lb allows to filter dataset for a specific class, not used
def _get_sampler(
    dataset: Dataset, val_ratio: Optional[float], shuffle: bool, max_items: Optional[int], apex: apex_utils.ApexUtils
