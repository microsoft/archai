...
_PREFIX_PATH = "_copy:"  # For copy node value command (must be scalar)
...
def _resolve_all(root_d: MutableMapping, cur: MutableMapping, cur_path: str, prev_paths: set):
    ...
    for k in cur.keys():
        ...
        if rpath:
            cur[k] = _resolve_path(root_d, _rel2full_path(_join_path(cur_path, k), rpath), prev_paths)
...
def _resolve_path(root_d: MutableMapping, path: str, prev_paths: set) -> Any:
    ...
    for part in path.split("/"):
        ...
        if isinstance(d, Mapping):
            ...
            raise KeyError(
                f'Path "{path}" cannot be resolved because "{cur_path}" is not a dictionary so "{part}" cannot exist in it'
            )
