...
from archai.trainers.coin_betting_optimizer import CocobBackprop
...
def create_optimizer(conf_opt: Config, params) -> Optimizer:
    ...
    if not math.isnan(decay_bn):
        ...
        params = [{"params": bn_params, "weight_decay": decay_bn}, {"params": rest_params, "weight_decay": decay}]

    if optim_type == "sgd":
        return SGD(params, lr=lr, momentum=conf_opt["momentum"], weight_decay=decay, nesterov=conf_opt["nesterov"])
    elif optim_type == "adam":
        return Adam(params, lr=lr, betas=conf_opt["betas"], weight_decay=decay)
    elif optim_type == "cocob":
        return CocobBackprop(params, alpha=conf_opt["alpha"])
    elif optim_type == "salsa":
        return statopt.SALSA(params, alpha=conf_opt["alpha"])
    else:
...
def ensure_pytorch_ver(min_ver: str, error_msg: str) -> bool:
    ...
    for i, j in zip(tv, req):
        ...
        if i < j:
            if error_msg:
                raise RuntimeError(
                    f"Minimum required PyTorch version is {min_ver} but installed version is {torch.__version__}: {error_msg}"
                )
            return False
    return True
...
def create_lr_scheduler(
    conf_lrs: Config, epochs: int, optimizer: Optimizer, steps_per_epoch: Optional[int]
    ...
    if conf_lrs is not None:
        ...
        if lr_scheduler_type == "cosine":
            scheduler = lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=epochs - warmup_epochs, eta_min=conf_lrs["min_lr"]
            )
        elif lr_scheduler_type == "multi_step":
            scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=conf_lrs["milestones"], gamma=conf_lrs["gamma"])
        elif lr_scheduler_type == "pyramid":
            scheduler = _adjust_learning_rate_pyramid(optimizer, epochs - warmup_epochs, get_optim_lr(optimizer))
...
def param_size(module: nn.Module, ignore_aux=True, only_req_grad=False):
    ...
    return np.sum(
        ...
        if (not ignore_aux or ("auxiliary" not in name)) and (not only_req_grad or v.requires_grad)
    )
...
def drop_path_(x, drop_prob, training):
    if training and drop_prob > 0.0:
        ...
        #   as per keep_prob.
        mask = torch.FloatTensor(x.size(0), 1, 1, 1).bernoulli_(keep_prob).to(device=x.device)
