...
class Evaluate(EnforceOverrides):
    def evaluate(self, conf_eval:Config, model_desc_builder:ModelDescBuilder):
        ...
        model = self._create_model(conf_eval, model_desc_builder)
    ...
    def _helper_train_desc(self, model, conf_train, conf_checkpoint, conf_loader, resume, metric_filename, model_filename, metrics_stats_filename, common_state):
        ''' Train given a model '''
    ...
    def _plot_model_gallery(self, metric_stats_all: List[MetricsStats])->None:
    ...
    def eval_archs(self, conf_eval:Config, model_desc_builder:ModelDescBuilder):
        ...
        for model_desc_filename in files:
            ...
            model = self._create_model(conf_eval, model_desc_builder, final_desc_filename=model_desc_filename, full_desc_filename=full_desc_filename)
            # number of cells and number of reductions don't obey a rule then model creation will fail
            if not model:
                continue

            this_id = self._helper_train_desc.remote(model, conf_train, conf_checkpoint, conf_loader, resume, metric_filename, model_filename, metrics_stats_filename, common.get_state())
    ...
    def _create_model(self, conf_eval:Config, model_desc_builder:ModelDescBuilder, final_desc_filename=None, full_desc_filename=None)->nn.Module:
        # region conf vars
