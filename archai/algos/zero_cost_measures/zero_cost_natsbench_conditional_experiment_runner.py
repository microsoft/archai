...
from .zero_cost_natsbench_conditional_evaluator import ZeroCostNatsbenchConditionalEvaluater

from nats_bench import create

class ZeroCostConditionalNatsbenchExperimentRunner(ExperimentRunner):
    ...
    def run_eval(self, conf_eval:Config)->EvalResult:
        ...
        # conditional zerocost evaluation
        # here we train regularly until a certain threshold accuracy is hit
        # then we compute the zero cost measures
        # -------------------------------------------
        # change relevant parts of conf_eval to ensure that evaluater 
        # doesn't resume from checkpoints created by other evalers and saves 
        # models to different names as well
        conf_eval['full_desc_filename'] = '$expdir/zerocost_cond_eval_full_model_desc.yaml'
        conf_eval['metric_filename'] = '$expdir/zerocost_cond_eval_train_metrics.yaml'
        conf_eval['model_filename'] = '$expdir/zerocost_cond_model.pt'
        
        if conf_eval['checkpoint'] is not None:
            conf_eval['checkpoint']['filename'] = '$expdir/zerocost_cond_checkpoint.pth'

        logger.pushd('zerocost_cond_evaluate')
        zerocost_cond_evaler = ZeroCostNatsbenchConditionalEvaluater()
        conf_eval_freeze = deepcopy(conf_eval)
        zerocost_cond_eval_result = zerocost_cond_evaler.evaluate(conf_eval_freeze, model_desc_builder=self.model_desc_builder())
        logger.popd()

        return zerocost_cond_eval_result