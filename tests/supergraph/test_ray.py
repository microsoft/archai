...
def train_test_dist():
    ...
    while len(result_ids):
        ...
        print(f"result={metrics.run_metrics.epochs_metrics[-1].top1.avg}, " f"time={time.time()-start}")


if __name__ == "__main__":
    ray.init(num_gpus=1)
    print("ray init done")
    common.common_init(config_filepath="confs/algos/darts.yaml", param_args=["--common.experiment_name", "resnet_test"])
