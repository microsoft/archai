...
def parse_args():
    ...
    parser.add_argument("--torch_model_path", type=str, help="Path to the PyTorch model/checkpoint file.")

    parser.add_argument("--onnx_model_path", type=str, help="Path to the output ONNX model file.")
    ...
    parser.add_argument("--opset_version", type=int, default=11, help="Version of ONNX operators.")

    parser.add_argument("--opt_level", type=int, default=0, help="Level of the ORT optimization.")

    parser.add_argument("--num_heads", type=int, default=8, help="Number of attention heads (for fusion).")

    parser.add_argument("--optimization", action="store_true", help="Applies optimization to the exported model.")

    parser.add_argument(
        "--quantization", action="store_true", help="Applies dynamic quantization to the exported model."
    )
...
if __name__ == "__main__":
    ...
    export_onnx_from_torch(
        model, model_config, model_type, onnx_model_path, share_weights=True, opset_version=opset_version
    )

    # Whether optimization should be applied
    if optimization:
        ort_model_path = optimize_onnx(model_type, onnx_model_path, num_heads=num_heads, opt_level=opt_level)
