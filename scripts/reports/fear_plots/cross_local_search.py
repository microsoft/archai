...
def main():
    parser = argparse.ArgumentParser(description='Cross Experiment Local Search Plots')
    parser.add_argument('--dataset', type=str, default='natsbench_cifar10',
                        help='dataset on which experiments have been run')
    parser.add_argument('--conf-location', type=str, default='scripts/reports/fastarchrank_plots/cross_local_search.yaml', 
    ...
    fig.update_layout(title_text="Duration vs. Max. Accuracy Local Search", 
                    xaxis_title="Duration (s)", 
                    yaxis_title='Avg. Top-1 Max Accuracy')

    savename_html = os.path.join(exp_folder, f'{args.dataset}_local_search.html')
