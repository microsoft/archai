# Copyright: https://github.com/HazyResearch/state-spaces/blob/main/src/models/sequence/ss/s4.py
#
...
class GConv(nn.Module):
    ...
    def __init__(
        ...
        **kernel_args,
        ...
        transposed: (B H L) if true else (B L H). chooses backbone axis ordering of (B, L, H) or (B, H, L) [B=batch size, L=sequence length, H=hidden dimension]
        ...
        for _ in range(self.num_scales):
            if "randn" in mode:
                ...
                kernel = nn.Parameter(
                    torch.cat(
                        [
                            torch.cos(torch.linspace(0, 2 * i * math.pi, self.kernel_dim)).expand(
                                channels, 1, self.kernel_dim
                            )
    ...
    def forward(self, u, return_kernel=False):
        ...
        if "sum" in self.mode:
            for i in range(self.num_scales):
                kernel = F.pad(
                    ...
                    (
                        0,
                        self.kernel_dim * 2 ** (self.num_scales - 1 + self.init_scale)
        ...
        if "learnable" in self.mode:
            k = k * torch.exp(
                -self.decay.view(1, -1, 1) * torch.log(torch.arange(k.size(-1), device=k.device) + 1).view(1, 1, -1)
            )
